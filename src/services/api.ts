...
class ApiService {
  ...
  async getMenuItems(onlyAvailable = true): Promise<BackendMenuItem[]> {
    return this.request<BackendMenuItem[]>(`/menu?onlyAvailable=${onlyAvailable}`);
  }
  ...
  async updateOrderStatus(id: number, status: 'PENDING' | 'IN_PROGRESS' | 'DONE' | 'CANCELLED'): Promise<BackendOrder> {
  ...
  async createTable(tableData: { tableNumber: number; capacity?: number }): Promise<BackendTable> {
    return this.request<BackendTable>('/tables', {
      method: 'POST',
  ...
  async createSession(qrCodeToken: string, meta?: any): Promise<BackendSession> {
...
export const convertBackendMenuItemToFrontend = (backendItem: BackendMenuItem) => ({
  id: backendItem.id,
